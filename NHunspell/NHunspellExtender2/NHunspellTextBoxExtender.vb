...
Public Class NHunspellTextBoxExtender
    ...
    Public Shared Event GetSentencesEvent As GetSentencesEventHandler
    Friend Shared customThreadPoolList As List(Of CustomThreadPool)
    Friend customThreadPool As CustomThreadPool
...
#Region "Private Classes"
    ...
    Private Class CustomPaintTextBox
        ...
        Private Sub CustomPaint() 'ByVal sender As Object, ByVal e As DoWorkEventArgs)
            ...
            Dim draw = False
            Dim lockObject = mySpellCheckControl.LockObject
            ...
            Using lockObject.Lock
                ...
                If mySpellCheckControl.HasSpellingErrors Then
...
GetNextLine:
                ...
                If mySpellCheckControl.GrammarEdits.Where(Function(g As GrammarEdit) Not g.HandledOrIgnored).Count() > 0 Then
                    ...
                    For Each grammarEdit In mySpellCheckControl.GrammarEdits.Where(Function(g As GrammarEdit) Not g.HandledOrIgnored)

                        startPoint = parentTextBox.GetPositionFromCharIndex(grammarEdit.StartGrammar)
                        endPoint = parentTextBox.GetPositionFromCharIndex(grammarEdit.EndGrammar)

                        Dim offsets As Point = GetOffsets(parentTextBox, grammarEdit.StartGrammar, grammarEdit.EndGrammar, tempRTB)
        ...
        Private Function GetOffsets(ByRef curTextBox As TextBoxBase, ByVal startingIndex As Integer,
            ...
            Dim index = Math.Min(curTextBox.Text.Length - 1, startingIndex + (endingIndex - startingIndex))

            If index > 0 Then
                offsetX = textBoxGraphics.MeasureString(curTextBox.Text(index),
                                                    fontToUse).Width
        ...
        ''' <remarks></remarks>
        Private Sub DrawWave(newPen As Pen, ByVal StartOfLine As Point, ByVal EndOfLine As Point)
...
#Region "Enabled"
    ...
    Public Shared Sub DisposeThreadPools()

        For Each threadPool In customThreadPoolList
            threadPool.Dispose()
        Next
    ...
    Public Sub SetSpellCheckEnabled(ByVal extendee As Control, ByVal Input As Boolean)
        ...
        If controlEnabled(extendee) Is Nothing Then
            ...
            If customThreadPoolList Is Nothing Then
                customThreadPoolList = New List(Of CustomThreadPool)()
            End If
...
#Region "TextBox Events"
    ...
    Private Sub TextBox_TextChanged(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Dim textBox = CType(sender, TextBoxBase)
...
#Region "ContextMenu"
    ...
    Private lastTextBox As TextBoxBase
    Private resetEvent As Threading.ManualResetEvent
    ...
    Private Sub ContextMenu_Opening(ByVal sender As System.Object,
        ...
        Dim spellChecker = CType(mySpellCheckers(ownerTextBox), SpellCheckControl)
        Dim lockObject = spellChecker.LockObject

        Using lockObject.Lock

            'first see if there are any spelling errors
            If Not spellChecker.HasSpellingErrors And Not spellChecker.HasGrammarErrors Then
                If CType(sender, ContextMenuStrip).Items.Count <> 0 Then e.Cancel = False
            ...
            If Not spellChecker.IsPartOfSpellingError(charIndex) Then

                If spellChecker.IsPartOfGrammarError(charIndex) Then
                    SetupGrammarContextMenu(CType(sender, ContextMenuStrip), charIndex)
                    e.Cancel = False
                    Return
                End If
    ...
    Private Sub SetupGrammarContextMenu(menu As ContextMenuStrip, charIndex As Integer)

        Dim grammarEdit As GrammarEdit
        ...
        Using lockObject.Lock
            ...
            Dim list = spellChecker.GrammarEdits.Where(Function(g As GrammarEdit) Not g.HandledOrIgnored And charIndex.IsBetween(g.StartGrammar, g.EndGrammar)).ToList()

            For Each grammarEdit In list

                item = menu.Items.Add(grammarEdit.Result.Correction.Replace("..", "."))
                item.Tag = grammarEdit

                AddHandler item.Click, New EventHandler(AddressOf GrammarItem_Click)

            Next

            item = menu.Items.Add("Ignore")
            item.Tag = list

        End Using

        AddHandler item.Click, New EventHandler(AddressOf GrammarItemIgnore_Click)

    End Sub

    Private Sub GrammarItemIgnore_Click(sender As Object, e As EventArgs)

        Dim item As ToolStripMenuItem = sender
        Dim list = CType(item.Tag, List(Of GrammarEdit))
        Dim spellChecker = CType(mySpellCheckers(ownerTextBox), SpellCheckControl)
        Dim lockObject = spellChecker.LockObject
    ...
    Private Sub GrammarItem_Click(sender As Object, e As EventArgs)

        Dim item As ToolStripMenuItem = sender
        Dim grammarEdit = CType(item.Tag, GrammarEdit)
        Dim add As Integer
        Dim length As Integer
        Dim spellChecker = CType(mySpellCheckers(ownerTextBox), SpellCheckControl)
        Dim lockObject = spellChecker.LockObject
        Dim editsList As List(Of Edit)

        Using lockObject.Lock
            editsList = grammarEdit.Result.Edits.ToList()
        End Using

        For Each edit In editsList
    ...
    Private Sub IgnoreSelectedWord(ByVal callingTextBoxName As String, ByVal LeftLocation As Integer,
        ...
        Using lockObject.Lock
            ...
            For Each currentRange In spellChecker.GetSpellingErrorRanges
    ...
    Private Sub timer_Tick(sender As Object, e As EventArgs) Handles timer.Tick
        ...
        Dim text = lastTextBox.Text
        Dim args As New GetSentenceEventArgs(text)
        Dim result As GrammarBotResult
        Dim spellChecker = CType(mySpellCheckers(lastTextBox), SpellCheckControl)
        Dim lockObject = spellChecker.LockObject

        If resetEvent Is Nothing Then
            resetEvent = New Threading.ManualResetEvent(False)
        ElseIf Not resetEvent.WaitOne(1) Then
            Return
        End If

        timer.Stop()

        RaiseEvent GetSentencesEvent(Me, args)

        If Not args.Sentences Is Nothing AndAlso args.Sentences.Length > 0 Then

            customThreadPool.QueueUserTask(Sub()

                                               Try

                                                   For Each sentence In args.Sentences
                                                       ...
                                                       Using lockObject.Lock
                                                           ...
                                                           If result.Edits.Count > 0 And result.Correction <> sentence Then

                                                               Try

                                                                   Dim index = lastTextBox.GetSafe(Function(c) c.Text.IndexOf(sentence))
                                                                   Dim startGrammar = index + result.Edits.Min(Function(e2) e2.Start)
                                                                   Dim endGrammar = Math.Min(index + result.Edits.Max(Function(e2) e2.End), lastTextBox.GetSafe(Function(c) DirectCast(c, System.Windows.Forms.TextBoxBase).TextLength))
                                                                   Dim grammarEdit As GrammarEdit
                                                                   ...
                                                                   If endGrammar = startGrammar Then
                                                                       startGrammar -= 1
                                                                   End If

                                                                   grammarEdit = spellChecker.GrammarEdits.SingleOrDefault(Function(g As GrammarEdit) g.StartGrammar = startGrammar And g.EndGrammar = endGrammar And g.Sentence = sentence)

                                                                   If grammarEdit Is Nothing Then

                                                                       grammarEdit = New GrammarEdit(startGrammar, endGrammar, sentence, result)

                                                                       For Each oldEdit In spellChecker.GrammarEdits.Where(Function(g As GrammarEdit) g.StartGrammar.IsBetween(grammarEdit.StartGrammar, grammarEdit.EndGrammar) Or g.EndGrammar.IsBetween(grammarEdit.StartGrammar, grammarEdit.EndGrammar)).ToList()
                                                                           spellChecker.GrammarEdits.Remove(oldEdit)
                                                                       Next

                                                                       spellChecker.GrammarEdits.Add(grammarEdit)

                                                                       lastTextBox.DelayInvoke(1, Sub()
                                                                                                      lastTextBox.Invalidate()
                                                                                                  End Sub)
                                                   ...
                                                   Using lockObject.Lock
                                                       For Each edit In spellChecker.GrammarEdits.ToList()

                                                           If Not text.Contains(edit.Sentence) Then
                                           ...
                                           End Sub, Sub(ByVal taskStatus As TaskStatus)

                                                        If Not taskStatus.Success Then
                                                            DebugUtils.Break()
                                                        End If

                                                    End Sub)

            While Not resetEvent.WaitOne(1)
