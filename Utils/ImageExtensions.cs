...
namespace Utils
{
    ...
    {
        public static PropertyItem CreatePropertyItem(this Image image, ImagePropertyTag tag)
        {
            ...
            var item = (PropertyItem) constructor.Invoke(new object[0]);
        ...
        public static Image GetThumbnailImage(this Image image, int thumbWidth, int thumbHeight = -1)
        {
            if (thumbHeight == -1)
            {
                var aspect = image.Width.As<float>() / image.Height.As<float>();

                return image.GetThumbnailImage(thumbWidth, (int)(20f / aspect), GetThumbnailImageAbortCallback, IntPtr.Zero);
            }
            else
            {
                return image.GetThumbnailImage(thumbWidth, thumbHeight, GetThumbnailImageAbortCallback, IntPtr.Zero);
            }
        }

        private static bool GetThumbnailImageAbortCallback()
        {
            return false;
        }
    }

    public enum ImagePropertyTag
    {
