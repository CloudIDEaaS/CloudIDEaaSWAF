...
export class HierarchyComponent implements OnInit {
  ...
  ngDoCheck(): void {
    ...
    if (this.nodes !== this.nodeCheck) {

      this.removeAll();

      this.nodeCheck = this.nodes;
  ...
  updateHierarchy() {
    ...
    let that = this,
      ...
      chartContainer = <any>document.querySelector("#chart-container");
  ...
  _one(el, type, listener, self = null) {
    ...
    if (
      ...
      let sibs = this._siblings(temp),
        sibsVisible = sibs.length
          ? !sibs.some((el: any) => el.classList.contains("hidden"))
          : false;
  ...
  _hoverNode(event) {
    ...
    if (event.type === "mouseenter") {
      ...
      Array.from(node.querySelectorAll(":scope > .edge")).forEach((el: any) => {
        el.classList.remove(
          "fa-chevron-up",
          "fa-chevron-down",
          "fa-chevron-right",
  ...
  showParent(node) {
    ...
    let temp = this._prevAll(
      this._closest(node, (el: any) => el.classList.contains("nodes"))
    );
  ...
  showSiblings(node, direction = null) {
    ...
    siblings.forEach(sib => {
      Array.from(sib.querySelectorAll(".node")).forEach((node: any) => {
  ...
  hideSiblings(node, direction = null) {
    let nodeContainer = this._closest(node, el => el.nodeName === "TABLE")
      .parentNode,
  ...
  addChildren(node, data) {
    ...
    this._buildChildNode.call(
      this,
      this._closest(node, el => el.nodeName.toLowerCase() === "table"),
      data,
      () => {
  ...
  _clickBottomEdge(event) {
    ...
    if (childrenState.exist) {
      ...
      if (
        Array.from(temp.querySelectorAll(".node")).some((node: any) => {
    ...
    if (nodeChart.parentNode.nodeName === "TD") {
      ...
      that.buildHierarchy.call(that, that.chart, nodeData, 0, () => {
        if (++nodeCount === siblingCount) {
          let temp =
            nodeChart.nextElementSibling.children[3].children[insertPostion],
  ...
  removeAll() {
    ...
    if (this.chart) {
      ...
      let chart = this.chart.querySelector("table");
      let node = chart.querySelector(".node");
  ...
  removeNodes(node) {
    let parent = this._closest(node, el => el.nodeName === "TABLE").parentNode,
      sibs = <Array<any>>this._siblings(parent.parentNode);
  ...
  _clickToggleButton(event) {
    let that = this,
      ...
      descendants = <Array<any>>(
        Array.from(descWrapper.querySelectorAll(".node"))
      ),
      children = Array.from(descWrapper.children).map((item: any) =>
  ...
  _onDragEnd(event) {
    Array.from(this.chart.querySelectorAll(".allowedDrop")).forEach(function (
      el: any
    ) {
  ...
  _onDrop(event) {
    ...
    if (!dropZone.parentNode.parentNode.nextElementSibling) {
      ...
      let dropColspan = parseInt(dropZone.parentNode.colSpan) + 2;
      ...
      let dropSibs = this._siblings(
        this._closest(dragged, function (el: any) {
          return el.nodeName === "TABLE";
        }).parentNode
      ).map((el: any) => el.querySelector(".node"));
    ...
    let dragColSpan = parseInt(dragZone.colSpan);
  ...
  buildHierarchy(appendTo, nodeData, level, callback = null) {
    // Construct the node

    let parentNodeData = appendTo.dataset;
  ...
  _loopChart(chart) {
    let node = chart.querySelector(".node");
    let subObj = JSON.parse(node.dataset.source);

    subObj = Object.assign(subObj, {
      children: []
    });
  ...
  getHierarchy(rootName) {
    ...
    nodes = this._loopChart(this.chart.querySelector("table"));
